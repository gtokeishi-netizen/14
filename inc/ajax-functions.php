...
// Removed duplicate action registration - gi_ajax_load_grants is already registered above
...
/**
 * =============================================================================
 * Missing AJAX Functions for Filter Functionality
 * These functions are called by unified-frontend.js but were missing
 * =============================================================================
 */

/**
 * Main filter grants AJAX function
 */
function gi_ajax_filter_grants() {
    // Nonce verification
    if (!wp_verify_nonce($_POST['nonce'] ?? '', 'gi_ajax_nonce')) {
        wp_send_json_error('セキュリティチェックに失敗しました');
    }
    ...
    if (!empty($filters['prefectures']) && is_array($filters['prefectures'])) {
        $tax_query[] = [
            'taxonomy' => 'grant_prefecture',
            'field' => 'slug',
            'terms' => array_map('sanitize_text_field', $filters['prefectures']),
            'operator' => 'IN'
        ];
    } elseif (!empty($filters['prefecture'])) {
        $tax_query[] = [
            'taxonomy' => 'grant_prefecture',
            'field' => 'slug',
            'terms' => [sanitize_text_field($filters['prefecture'])],
            'operator' => 'IN'
        ];
    }
    ...
    if (!empty($filters['municipalities']) && is_array($filters['municipalities'])) {
        $tax_query[] = [
            'taxonomy' => 'grant_municipality',
            'field' => 'slug',
            'terms' => array_map('sanitize_text_field', $filters['municipalities']),
            'operator' => 'IN'
        ];
        ...
        $tax_query[] = [
            'taxonomy' => 'grant_municipality',
            'field' => 'slug',
            'terms' => [sanitize_text_field($filters['municipality'])],
            'operator' => 'IN'
        ];
    }
    ...
    if (!empty($filters['difficulty'])) {
        $difficulties = is_array($filters['difficulty']) ? $filters['difficulty'] : [$filters['difficulty']];
        $meta_query[] = [
            'key' => 'application_difficulty',
            'value' => array_map('sanitize_text_field', $difficulties),
            'compare' => 'IN'
        ];
    }
    
    // Success rate
    if (!empty($filters['success_rate'])) {
        $meta_query[] = [
            'key' => 'success_rate_range',
            'value' => sanitize_text_field($filters['success_rate']),
            'compare' => '='
        ];
    }
    
    // Application method
    if (!empty($filters['application_method'])) {
        $methods = is_array($filters['application_method']) ? $filters['application_method'] : [$filters['application_method']];
        $meta_query[] = [
            'key' => 'application_method',
            'value' => array_map('sanitize_text_field', $methods),
            'compare' => 'IN'
        ];
    }
    ...
    // Sorting
    $sort = sanitize_text_field($_POST['sort'] ?? $filters['sort'] ?? 'date_desc');
    switch ($sort) {
        case 'date_asc':
            $args['orderby'] = 'date';
            $args['order'] = 'ASC';
    ...
    if ($query->have_posts()) {
        ...
        foreach ($query->posts as $post) {
            $grants_data[] = [
                ...
                'amount' => gi_safe_get_meta($post->ID, 'max_amount', ''),
                'deadline' => gi_safe_get_meta($post->ID, 'deadline_date', ''),
                'status' => gi_safe_get_meta($post->ID, 'application_status', ''),
            ];
        }
    }
    ...
    wp_send_json_success([
        ...
        'filters_applied' => !empty($tax_query) || !empty($meta_query),
        'query_vars' => $args // Debug info
    ]);
}

/**
 * Search grants AJAX function
 */
function gi_ajax_search_grants() {
    ...
    if (strlen($query) < 2) {
        wp_send_json_error('検索クエリが短すぎます');
    }
    
    // Use the same logic as filter grants but with search focus
    $filters = ['search' => $query];
    $_POST['filters'] = $filters;
    
    // Delegate to filter function
    gi_ajax_filter_grants();
}

// Removed duplicate gi_ajax_get_search_suggestions function - already defined at line 357

/**
 * Load more grants AJAX function (for infinite scroll)
 */
function gi_ajax_load_more_grants() {
    ...
    // Set page and delegate to filter function
    $_POST['page'] = $page;
    $_POST['filters'] = $filters;
...
add_action('wp_ajax_gi_filter_grants', 'gi_ajax_filter_grants');
add_action('wp_ajax_nopriv_gi_filter_grants', 'gi_ajax_filter_grants');
